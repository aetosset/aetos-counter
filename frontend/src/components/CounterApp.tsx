...
export default function CounterApp() {
  ...
  const [connectFns, setConnectFns] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        const { AppConfig, UserSession, showConnect, openContractCall } = await import('@stacks/connect');
        ...
        setConnectFns({ showConnect, openContractCall });
        ...
        setWalletReady(true); // Still show button
      }
    };
    
    // Delay to avoid race with wallet extensions
    setTimeout(init, 200);
  ...
  const callContract = async (functionName: string, args: any[] = []) => {
    if (!userData || !connectFns) return;
    
    setLoading(true);
    setTxId(null);
    
    try {
      await connectFns.openContractCall({
        contractAddress: CONTRACT_ADDRESS,
        contractName: CONTRACT_NAME,
        functionName,
        functionArgs: args,
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="max-w-md mx-auto">
          {!userData ? (
            ...
            >
              {walletReady ? 'Connect Wallet' : 'Loading...'}
            </button>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-between bg-gray-800/50 rounded-xl p-4 border border-gray-700">
                ...
                <button onClick={disconnect} className="text-red-400 hover:text-red-300 text-sm">
                  Disconnect
                </button>
              </div>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-4 px-6 bg-red-500/20 hover:bg-red-500/30 border border-red-500/50 rounded-xl font-semibold text-xl text-red-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  -
                </button>
                <button
                  onClick={increment}
                  disabled={loading}
                  className="flex-1 py-4 px-6 bg-green-500/20 hover:bg-green-500/30 border border-green-500/50 rounded-xl font-semibold text-xl text-green-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  +
                </button>
              </div>
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-3 px-6 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/50 rounded-xl font-semibold text-purple-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                +10
              </button>
              
              {loading && (
                <p className="text-center text-yellow-400 animate-pulse">Waiting for wallet...</p>
              )}
        ...
        <div className="text-center mt-16 text-gray-500 text-sm">
          ...
          <p className="mt-2">
            <a href="https://github.com/aetosset" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">
              GitHub
            </a>
            {' · '}
            <a href={`https://explorer.hiro.so/address/${CONTRACT_ADDRESS}?chain=mainnet`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">
              Contract
            </a>
          </p>
        </div>
